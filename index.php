...
if(preg_match_all("/^(list_bought_product)$/ui", $_GET['type'])){
    $query = "SELECT `id`, `product_id`, `user_id` FROM `bought_product`";
    $res_query = mysqli_query($connection, $query);
...
if(preg_match_all("/^(list_comments)$/ui", $_GET['type'])){
    $query = "SELECT `id`, `product_id`, `user_id`, `comment` FROM `comments`";
    $res_query = mysqli_query($connection, $query);
